...
import { buildLink, goBack, navigationToScreen } from "../utils/utils";
...
import { deleteGroup, leaveGroup } from "../redux/actions/commonActions";
...
const ChatDetails = (props) => {
  ...
  const leaveGroups = async (it) => {
    try {
      console.log(it());
      if (it() === "CONTACT") {
        let admin = {
          group_admin_id: data?.group?.group_admin_id,
          admin_name: data?.group?.group_admin_name,
        ...
        let recRefexist = (await recRef.get()).exists;
        console.log(isExist);
        if (!isExist) {
          adminRef
            .set({
            ...
            .then(() => {
              if (!recRefexist) {
                recRef
                  ...
                  .then(() => {
                    navigationToScreen(
                      CONSTANTS.SCREENLIST.PRIVATE_CHATS,
                      props,
                      {
                        data: admin,
                      }
        ...
        const res = await dispatch(leaveGroup(token?.id, data?.group?.id));
        if (res) {
          goBack(props, 1);
        }
      } else if (it() == "DELETE") {
        const res = await dispatch(deleteGroup(token?.id, data?.group?.id));
        if (res) {
          firestore()
            .collection("Groups")
            .doc(data?.group.id)
        ...
        const res = await buildLink(data?.group?.id);
        console.log(res);
      ...
      <View style={[styles.chatHeaderStyle]}>
        ...
        <View style={{ width: wp("35%") }}>
          <DropDown
            ...
            data={
              token?.id == data?.group.group_admin_id
                ? [
                ...
                : [
                    ...
                    { label: "Invite", value: "INVITE" },
                    { label: "Message admin", value: "CONTACT" },
                  ] || []
            }
            showIcon={true}
            value={option}
            setValue={(it) => leaveGroups(it)}
